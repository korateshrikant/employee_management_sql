...
sql_server_config = {
    'Driver': '{SQL Server}',
    'Server': 'DESKTOP-V9NJ13A',
    'Database': 'employee_management_system'
}

# making Connection
con = sql_conn = pyodbc.connect(**sql_server_config)
...
def Promote_Employee():
	...
	if(check_employee(Id) == False):
		...
		# with given Id
		sql = 'select salary from dbo.Employee where id=?'
		data = (Id,)
		c = con.cursor()
		...
		# given Id
		sql = 'update dbo.Employee set salary=? where id=?'
		d = (t, Id)
...
def Remove_Employ():
	...
	if(check_employee(Id) == False):
		...
		sql = 'delete from dbo.Employee where id=?'
		data = (Id,)
		c = con.cursor()
...
def check_employee(employee_id):
	...
	# rom employee Table
	sql = 'select * from dbo.Employee where id=?'
